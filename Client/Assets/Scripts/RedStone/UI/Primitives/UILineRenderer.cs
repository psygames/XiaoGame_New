...
namespace Hotfire.UI
{
	...
	public class UILineRenderer : UIPrimitiveBase
	{
		...
		protected override void OnPopulateMesh(VertexHelper vh)
		{
			...
			var segments = ListPool<UIVertex[]>.Get();
			if (LineList)
			{
            ...
            for (var i = 0; i < segments.Count; i++)
			{
				...
				var count = segments [i].Length;
				for (int j = 0; j < count; ++j)
				{
					var pos = segments [i] [j].position;
					if (i == 0 && j == 0)
					{
						maxValue = minValue = pos;
					}
					minValue = Vector2.Min (minValue, pos);
					maxValue = Vector2.Max (maxValue, pos);
				}
				vh.AddUIVertexQuad(segments[i]);
			}
			sizeDelta = (maxValue - minValue);
			segments.ReleaseToPool ();
		}
