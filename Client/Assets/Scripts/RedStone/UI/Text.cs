...
namespace Hotfire.UI
{
    ...
    public class Text : UnityEngine.UI.Text
    {
        ...
        List<Hotfire.UI.Image> m_imageList = new List<Hotfire.UI.Image>();
        ...
        {
            var list = GetComponentsInChildren<Hotfire.UI.Image>(true);
            m_imageList.Clear();
            if (list != null)
            {
        ...
        void LateUpdate()
        {
            if (spriteTagChanged)
            {
                ...
                {
                    var image = new GameObject("Image").AddComponent<Hotfire.UI.Image>();
