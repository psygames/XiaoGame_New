...
namespace uTools
{
	public abstract class uTweener : MonoBehaviour
	{
		...
		public Action<GameObject> onFinishedAction = null;
		...
		void Update ()
		{
			...
			if (style == Style.PingPong || style == Style.PingPongOnce)
			{
			...
			if (((style == Style.Once) && (duration == 0f || mFactor > 1f || mFactor < 0f))
			    || ((style == Style.PingPongOnce) && (duration == 0f || mPonged)))
			{
				...
				if (current != this)
				{
					...
					{
						onFinishedAction.Invoke (gameObject);
					}
		...
		public void SetOnFinishedAction(Action<GameObject> finishedCallBack)
		{
			onFinishedAction = finishedCallBack;
		}
		...
		static public void Stop<T> (GameObject go) where T : uTweener
		{
			T comp = go.GetComponent<T> ();
			if (comp != null)
				comp.enabled = false;
		}
		static public T Begin<T> (GameObject go, float duration,Action<GameObject> finishHandler=null) where T : uTweener
		{
			T comp = go.GetComponent<T> ();
#if UNITY_FLASH
		if ((object)comp == null) comp = (T)go.AddComponent<T>();
			...
			comp.enabled = true;
            comp.onFinishedAction = finishHandler;
			return comp;
		}
