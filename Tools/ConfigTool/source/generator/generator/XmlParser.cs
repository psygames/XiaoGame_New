...
namespace xmlparser
{
    class XmlParser
    {
        ...
        public Dictionary<string, List<MyClassMember>> GeneMembersByXmlText(string tablename, string xmlText)
        {
            Console.WriteLine(tablename);
            Dictionary<string, List<MyClassMember>> dictionary = new Dictionary<string, List<MyClassMember>>();
            m_parser.LoadXml(xmlText);
            SecurityElement el_0 = m_parser.ToXml();
            int sheetIndex = 0;
            foreach (SecurityElement el_1 in el_0.Children)
            {
                if (sheetIndex >= m_maxSheetCheckCount)
                    break;
                if (el_1.Tag != TAG_WORKSHEET)
                    continue;
                ...
                foreach (SecurityElement el_2 in el_1.Children)
                {
                    if (el_2.Tag != TAG_TABLE || el_2.Children == null || el_2.Children.Count <= 0)
                        continue;
                    List<MyClassMember> mem = new List<MyClassMember>();
                    int rowIndex = 0;
                    int cellIndex = 0;
                    foreach (SecurityElement row in el_2.Children)
                    {
                        if (row.Tag != TAG_ROW || row.Children == null || row.Children.Count <= 0)
                            continue;
                        cellIndex = 0;
                        foreach (SecurityElement cell in row.Children)
                        {
                            ...
                            if (!string.IsNullOrEmpty(idxStr))
                            {
                                ...
                                {
                                    mem[cellIndex].Description = "没有描述信息";
                                    ++cellIndex;
                                }
                            }
                            ...
                            {
                                mem.Add(new MyClassMember());
                                mem[cellIndex].Name = data;
                            }
                            else if (rowIndex == ROW_INDEX_TYPE)
                            {
                                mem[cellIndex].Type = data;
                            }
                            else if (rowIndex == ROW_INDEX_DESCRIPTION)
                            {
                                mem[cellIndex].Description = string.IsNullOrEmpty(data) ? "没有描述信息" : data;
                            }
                            cellIndex++;
                        }

                        if (rowIndex >= ROW_INDEX_DATA_BEGIN)
                        {
                            // check null description
                            foreach (MyClassMember member in mem)
                            {
                                if (member.Description == null)
                                {
                                    member.Description = "没有描述信息";
                                }
                            }

                            dictionary.Add(el_1.Attributes[KEY_SHEET_NAME].ToString(), mem);
                            break;
                        }
                        rowIndex++;
                    }
